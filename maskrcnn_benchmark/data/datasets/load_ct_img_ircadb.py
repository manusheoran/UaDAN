...
def load_prep_img(data_dir, imname, spacing, slice_intv, do_clip=False, num_slice=3, is_train=False, is_fewshot_dl_hu=False):
    """load volume, windowing, interpolate multiple slices, clip black border, resize according to spacing"""
    data_dir = '/data/LITS/3Dircadb/npy_images'
    #data_dir = '/gdrive/MyDrive/TCS/KiTS/npy_images_uda'
    im, mask = load_multislice_img_16bit_png(data_dir, imname, slice_intv, do_clip, num_slice)

    if not is_fewshot_dl_hu:
        im = windowing(im, cfg.INPUT.WINDOWING_liver)  # cfg.INPUT.WINDOWING
