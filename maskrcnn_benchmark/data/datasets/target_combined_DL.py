...
import logging
import json
from maskrcnn_benchmark.data.datasets.load_ct_img import load_prep_img as load_prep_img_dl
...
class DL_target_combinedDataset(object):
    ...
    def __init__(
        ...
        self.logger.info('DeepLesion and combined target fewshot  %s num_images: %d' % (split, self.num_images))
    ...
    def __getitem__(self, index):
        ...
        if is_train and '.png' in image_fn and cfg.INPUT.DATA_AUG_3D is not False :   ########### major change
            temp = image_fn.split('.png')[0].split('_')
            #print( image_fn.split('.png'))
            image_fn = temp[0]+'_' + temp[1]+'_' + temp[2]+'/' + temp[3] + '.png'

        #print('image_fn',image_fn)
        
        if '.png' in image_fn:     ########### major change
          im, im_scale, crop = load_prep_img_dl(self.data_path, image_fn, spacing, slice_intv,cfg.INPUT.IMG_DO_CLIP, num_slice=num_slice, is_train=is_train)
        
        elif 'case' in image_fn:   ########### major change
